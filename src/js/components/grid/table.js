...
let Row = Base.extend({
    ...
    didRender: function() {
        // 操作列的列数
        this.ACTION_COLUMN_NUM = -1;
        _.each(this.columns, function(cell, index) {
            if(cell.type === 'action'){
                this.ACTION_COLUMN_NUM = index + 1;
            }
        ...
        switch (type) {
            ...
            case 'action':
                ...
                _.each(cell.items, function(action) {
                    item = _this._createActionItem(action);
                    $td.append(item);
                });
                break;
            case 'text':
                ...
                if (cell.formatter && _.isFunction(cell.formatter)) {
                    value = cell.formatter(value, _this, $td);
                }
                item = value;
                $td.append(item);
                break;
    ...
    addAction: function(actionConfig){
        if (this.ACTION_COLUMN_NUM > -1) {
            let item = this._createActionItem(actionConfig);
            let actionCell = this.$el.find('td:nth-child(' + this.ACTION_COLUMN_NUM + ')');
            actionCell.append(item);
        }
    },

    _createActionItem: function(actionConfig){
        let text = actionConfig.text || '';
        if(actionConfig.formatter && _.isFunction(actionConfig.formatter)) {
            text = actionConfig.formatter(this.data, this);
        }
        let item = $('<a>' + text + '</a>');
        item.on('click', () => {
            actionConfig.onClick && actionConfig.onClick(this.data, this);
        });
        return item;
    },

    willDestroy: function(){
        delete this.ACTION_COLUMN_NUM;
    }
});
...
export default Base.extend({
    ...
    loadData: function(pageHandler) {
        if (this.endPoint && this.remoteMethod) {
            ...
            Scalar.getRequest(this.endPoint, this.remoteMethod)({
                data: params
            ...
            func().then(this.createBody.bind(this));
        }
    },
    ...
    calculateBodyHeight: function() {
        ...
        // 外层容器的高度为0或者没有渲染完成时，不重新计算表格的高度
        if(grid.el.clientHeight === 0){
            return;
        }
        ...
        let tableBodyHeight = grid.el.clientHeight - total - 10;
